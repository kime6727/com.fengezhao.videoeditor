...
// 获取视频信息
$stmt = $pdo->prepare("SELECT `material_id`, `video_url`, `thumbnail_url`
...
if ($enableDownloadLimit === 'true') {
    ...
    // 调试日志
    error_log("video-text下载限制检查: userId=$userId, isVIP=" . ($isVIP ? 'true' : 'false') . ", freeLimit=$freeUserLimit, vipLimit=$vipUserLimit, userLimit=$userLimit, todayCount=$todayCount");

    // 检查限制（如果 userLimit 为 0，表示无限制）
    if ($userLimit > 0 && $todayCount >= $userLimit) {
        error_log("video-text下载限制检查失败: userId=$userId, userLimit=$userLimit, todayCount=$todayCount");
    ...
    error_log("video-text下载限制检查通过: userId=$userId, remaining=" . ($userLimit > 0 ? ($userLimit - $todayCount) : 9999));
}
...
try {
    ...
    echo jsonResponse(200, '下载成功', [
        'video_url' => absoluteMediaUrl($video['video_url']),
        'thumbnail_url' => absoluteMediaUrl($video['thumbnail_url']),
