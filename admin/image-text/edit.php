...
// 获取素材信息
$stmt = $pdo->prepare("SELECT * FROM `image_text_materials` WHERE `material_id` = ?");
...
// 获取图片列表
$stmt = $pdo->prepare("SELECT `image_url`, `sort` FROM `image_text_images`
                     WHERE `material_id` = ? ORDER BY `sort` ASC");
$stmt->execute([$materialId]);
$images = $stmt->fetchAll(PDO::FETCH_ASSOC);

// 获取文案列表
$stmt = $pdo->prepare("SELECT `content`, `sort` FROM `image_text_contents`
                     WHERE `material_id` = ? ORDER BY `sort` ASC");
$stmt->execute([$materialId]);
...
$stmt = $pdo->prepare("SELECT `category_id` FROM `category_relations`
                      WHERE `material_id` = ? AND `material_type` = 2");
$stmt->execute([$materialId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($materialId)) {
        ...
        try {
            ...
            // 删除旧文案
            $stmt = $pdo->prepare("DELETE FROM `image_text_contents` WHERE `material_id` = ?");
            $stmt->execute([$materialId]);
                ...
                <div id="images-container">
                    <?php foreach ($images as $image): ?>
                        <div class="item-group">
                            <input type="url" name="image_urls[]" value="<?php echo htmlspecialchars($image['image_url']); ?>" required>
                            <button type="button" class="remove-btn" onclick="this.parentElement.remove()">删除</button>
                        </div>
                    <?php endforeach; ?>
                </div>
                <div class="hint">至少需要1张图片，最多9张</div>
            </div>
