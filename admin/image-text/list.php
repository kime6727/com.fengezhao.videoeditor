...
// 分类筛选
if (!empty($categoryId)) {
    $whereConditions[] = "itm.`material_id` IN (
        SELECT `material_id` FROM `category_relations`
        WHERE `category_id` = ? AND `material_type` = 2
    )";
    $params[] = $categoryId;
}
...
// 状态筛选
if ($status >= 0) {
    $whereConditions[] = "itm.`status` = ?";
    $params[] = $status;
}

// 关键词搜索（通过素材ID或文案内容）
if (!empty($keyword)) {
    $whereConditions[] = "(itm.`material_id` LIKE ? OR EXISTS (
        SELECT 1 FROM `image_text_contents` itc
        WHERE itc.`material_id` = itm.`material_id`
        AND itc.`content` LIKE ?
    ))";
    $searchParam = '%' . $keyword . '%';
    $params[] = $searchParam;
    $params[] = $searchParam;
...
// 获取素材列表
$sql = "SELECT itm.`material_id`, itm.`download_count`, itm.`like_count`, itm.`status`, itm.`created_at`
        FROM `image_text_materials` itm
        $whereClause
        ORDER BY itm.`$orderBy` $orderDir
        LIMIT ? OFFSET ?";
...
foreach ($materials as &$material) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM `image_text_images` WHERE `material_id` = ?");
    $stmt->execute([$material['material_id']]);
    $material['image_count'] = $stmt->fetchColumn();

    $stmt = $pdo->prepare("SELECT COUNT(*) FROM `image_text_contents` WHERE `material_id` = ?");
    $stmt->execute([$material['material_id']]);
    $material['content_count'] = $stmt->fetchColumn();

    // 获取第一张图片作为缩略图
    $stmt = $pdo->prepare("SELECT `image_url` FROM `image_text_images`
                          WHERE `material_id` = ? ORDER BY `sort` ASC LIMIT 1");
    $stmt->execute([$material['material_id']]);
    $material['thumbnail'] = $stmt->fetchColumn();

    // 获取分类信息
    $stmt = $pdo->prepare("SELECT c.`category_id`, c.`name`
                          FROM `categories` c
                          INNER JOIN `category_relations` cr ON c.`category_id` = cr.`category_id`
                          WHERE cr.`material_id` = ? AND cr.`material_type` = 2
...
// 获取总数
$countSql = "SELECT COUNT(*) FROM `image_text_materials` itm $whereClause";
    ...
    <div class="admin-layout">
        ...
        <div class="main-content">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <h1 style="margin: 0; font-size: 20px;">图片+文案素材管理</h1>
                <div>
                    <a href="add.php" style="display: inline-block; padding: 6px 12px; background: #667eea; color: white; text-decoration: none; border-radius: 4px; margin-left: 10px; font-size: 13px;">添加素材</a>
        ...
        <div class="filters">
            <div class="search-input">
                <label>搜索：</label>
                <input type="text" name="keyword" placeholder="输入ID或文案内容搜索..." value="<?php echo htmlspecialchars($keyword); ?>"
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($material['thumbnail']): ?>
                            <img src="<?php echo htmlspecialchars($material['thumbnail']); ?>" class="thumbnail" alt="">
                        <?php else: ?>
                    ...
                    <td><?php echo $material['image_count']; ?> 张</td>
                    <td><?php echo $material['content_count']; ?> 条</td>
