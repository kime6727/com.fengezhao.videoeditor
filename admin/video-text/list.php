...
// 分类筛选
if (!empty($categoryId)) {
    $whereConditions[] = "vtm.`material_id` IN (
        SELECT `material_id` FROM `category_relations`
        WHERE `category_id` = ? AND `material_type` = 3
    )";
    $params[] = $categoryId;
}
...
// 状态筛选
if ($status >= 0) {
    $whereConditions[] = "vtm.`status` = ?";
    $params[] = $status;
}

// 关键词搜索（通过素材ID或文案内容）
if (!empty($keyword)) {
    $whereConditions[] = "(vtm.`material_id` LIKE ? OR EXISTS (
        SELECT 1 FROM `video_text_contents` vtc
        WHERE vtc.`material_id` = vtm.`material_id`
        AND vtc.`content` LIKE ?
    ))";
    $searchParam = '%' . $keyword . '%';
    $params[] = $searchParam;
    $params[] = $searchParam;
...
$sql = "SELECT vtm.`material_id`, vtm.`video_url`, vtm.`thumbnail_url`,
               vtm.`download_count`, vtm.`like_count`, vtm.`status`, vtm.`created_at`
        FROM `video_text_materials` vtm
        $whereClause
        ORDER BY vtm.`$orderBy` $orderDir
        LIMIT ? OFFSET ?";
...
foreach ($materials as &$material) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM `video_text_contents` WHERE `material_id` = ?");
    ...
    $stmt = $pdo->prepare("SELECT c.`category_id`, c.`name`
                          ...
                          WHERE cr.`material_id` = ? AND cr.`material_type` = 3
...
// 获取总数
$countSql = "SELECT COUNT(*) FROM `video_text_materials` vtm $whereClause";
    ...
    <div class="admin-layout">
        ...
        <div class="main-content">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <h1 style="margin: 0; font-size: 20px;">视频+文案素材管理</h1>
                <div>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($material['thumbnail_url']): ?>
                            <img src="<?php echo htmlspecialchars($material['thumbnail_url']); ?>" class="thumbnail" alt="">
                        <?php else: ?>
